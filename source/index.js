...
function getOwnerInformaiton(event, cb) {
    console.log("retrieving caller informaiton");
...
try
{
    ...
    var table = "button-owner";

    var key = `${event.serialNumber}-${event.clickType}`;
    ...
    docClient.get(params, function(err, data) {
        if (err) {
            console.error(`Unable to read item. key=${key} Error JSON:`, JSON.stringify(err, null, 2));
            cb(err);
        } 
...
exports.handler = (event, context, callback) => {
    ...
    createTopic('aws-iot-button-sns-topic', (err, topicArn) => {
        ...
        getOwnerInformaiton(event, (err, owner) => {
            if (err) {
                return callback(err);
            }
            ...
            // publish message
            const params = {
                Message: `${event.serialNumber} -- processed by Greg and Lambda\nBattery voltage: ${event.batteryVoltage}`,
                Subject: `Hello ${owner.name} from your IoT Button ${event.serialNumber}: ${event.clickType}`,
                TopicArn: topicArn,
            };
